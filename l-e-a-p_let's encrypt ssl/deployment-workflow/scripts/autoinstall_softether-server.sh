...
uninstall_softether() {
    ...
    rm -f /var/lock/vpnserver 2>/dev/null
...
install_softether() {
    ...
    # [FIX] Validate wget download — exit immediately if download fails
    if ! wget "$download_url" -O "/opt/softether-vpnserver.tar.gz"; then
        echo -e "\033[1;31mError: Download failed. Please check your connection or the URL above.\033[0m" >&2
    ...
    # [FIX] Explicit chmod — avoid accidentally stripping execute bit from all binaries
    chmod 600 *
    chmod 700 vpncmd vpnserver
    # Set other known executables if present
    for bin in vpnbridge vpnclient vpnsmgr; do
        [ -x "$bin" ] && chmod 700 "$bin"
    done
...
# [FIX] Use /var/lock/ instead of /var/lock/subsys/ — subsys/ does not exist on Debian/Ubuntu
...
EOF
    ...
    # [FIX] Verify service actually started after systemctl start
    sleep 3
    if systemctl is-active --quiet vpnserver; then
        echo -e "\033[1;32mVPN service started successfully.\033[0m"
    else
        echo -e "\033[1;31mWarning: VPN service may not have started correctly. Check: journalctl -u vpnserver\033[0m" >&2
    fi
