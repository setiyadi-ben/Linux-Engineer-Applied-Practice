#!/bin/bash
# SoftEther VPN Client Installation Manager
...
# Usage: 
# sudo chmod +x ./autoinstall_softether-vpnclient.sh
# sudo ./autoinstall_softether-vpnclient.sh
...
check_installation() {
    if [ -d "/usr/local/vpnclient" ]; then
        echo -e "\033[1;32mSoftEther VPN Client is currently installed.\033[0m"
        return 0
    else
        echo -e "\033[1;33mSoftEther VPN Client is not installed.\033[0m"
        return 1
    fi
}
...
uninstall_softether() {
    echo -e "\n\033[1;31mUninstalling SoftEther VPN Client...\033[0m"
    
    # Stop and disable service
    systemctl stop vpnclient 2>/dev/null
    systemctl disable vpnclient 2>/dev/null
    rm -f /etc/systemd/system/vpnclient.service 2>/dev/null
    
    # Remove files
    rm -f /opt/vpnclient.sh 2>/dev/null
    rm -rf /usr/local/vpnclient 2>/dev/null
    
    # Remove extracted files in /opt
    find /opt -maxdepth 1 -type d -name 'vpnclient' -exec rm -rf {} + 2>/dev/null
    find /opt -maxdepth 1 -type f -name 'softether-vpnclient-*.tar.gz' -exec rm -f {} + 2>/dev/null
    rm -f /opt/softether-vpnclient.tar.gz 2>/dev/null
...
install_softether() {
    ...
    # Display menu
    echo ""
    echo "Available SoftEther VPN Client versions:"
    ...
    arch_list=$(curl -s "$base_url/Linux/SoftEther_VPN_Client/" | grep -Eo 'Linux/SoftEther_VPN_Client/[^/<>]+/' | sort -u)
    ...
    # Extract filename
    filename=$(curl -s "$base_url/$selected_arch" | grep -Eo 'softether-vpnclient[^"]+\.tar\.gz' | head -1)

    # Download the package
    echo "Downloading from the server..."
    download_url="$base_url/$selected_arch$filename"
    echo "Download URL: $download_url"
    wget "$download_url" -O "/opt/softether-vpnclient.tar.gz"
...
[Service]
ExecStart = /opt/vpnclient.sh start
ExecStop = /opt/vpnclient.sh stop
ExecReload = /opt/vpnclient.sh restart
Restart = always
Type = forking
...
EOF
    ...
    # Cleanup
    rm -f /opt/softether-vpnclient.tar.gz
...
# Main script logic
echo -e "\n\033[1;34mSoftEther VPN Client Installation Manager\033[0m"
echo "=============================================="
...
if [ $installed -eq 0 ]; then
    ...
    echo -e "\n\033[1;36m1) Install SoftEther VPN Client"
...
if check_installation; then
    ...
    systemctl status vpnclient --no-pager
    echo -e "\n\033[1;32mSetup complete! SoftEther VPN Client is running.\033[0m"
else
