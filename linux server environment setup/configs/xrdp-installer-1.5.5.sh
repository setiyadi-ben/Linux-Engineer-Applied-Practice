#!/bin/bash
#####################################################################################################
# Script_Name : xrdp-installer-1.5.5.sh
...
# Version : 1.5.5
# History : 1.5.5 - adding support to H264 encoding/decoding protocol (x264)
...
/bin/echo -e "\e[1;36m   
  !----------------------------------------------------------------!
  !             xrdp-installer script - Ver. $ScriptVer            !
  !           Support Ubuntu and Debian Based distribution         !        
  !                Written by Griffon - October 2025               !  
  !                      www.c-nergy.be                            ! 
  !                                                                !
  ! For Help and Syntax, type ./xrdp-installer-$ScriptVer.sh -h    !
  !----------------------------------------------------------------!
\e[0m"
/bin/echo -e "\e[1;38m   
  !----------------------------------------------------------------!
...
array=("Ubuntu 22.04" "Ubuntu 24.04" "Ubuntu 25.10" "Ubuntu 25.04" "Pop!_OS 20.04" "Pop!_OS 22.04" "Debian" "Mint" "LMDE" "Zorin" "elementary OS")
...
#For U25.10 - need to install xorg package....
exit 1
fi
}
...
#Detect Desktop Interface
if command -v plasmashell &> /dev/null; then
    echo "Kubuntu (KDE Plasma) detected"
    TempDE="KDE"
fi

if command -v gnome-shell &> /dev/null; then
    echo "Ubuntu GNOME detected"
    TempDE="Gnome"
fi

#Need to exclude Gnome 49
   # Ubuntu 25.10 - No Gnome 49 Support ...
    if [[ $codename = *"questing"* ]] && [[ $TempDE = *"Gnome"*  ]] 
    then 
        /bin/echo -e "\e[1;32m       |-| Cannot proceed installation...         \e[0m"
        echo
        /bin/echo -e "\e[1;31m  !--------------------------------------------------------------!\e[0m"
        /bin/echo -e "\e[1;31m  ! You are running Ubuntu 25.10 with Gnome 49.                  !\e[0m"   
        /bin/echo -e "\e[1;31m  ! xRDP does not support Wayland session with Gnome Desktop     !\e[0m"
        /bin/echo -e "\e[1;31m  ! The script is exiting...                                     !\e[0m"                
        /bin/echo -e "\e[1;31m  !--------------------------------------------------------------!\e[0m"
        echo
        exit
...
# Kubuntu 25.10 - Specific 
if [[ $codename = *"questing"* ]] && [[ $TempDE = *"KDE"*  ]]
then 
    /bin/echo -e "\e[1;32m       |-| Adding x11 support for KDE on Ubuntu 25.10...         \e[0m"
    echo
    sudo apt install startplasma-x11 -y 
fi
...
sudo apt-get -y install libmp3lame-dev curl libfuse3-dev libx11-dev libxfixes-dev libssl-dev libpam0g-dev libtool libjpeg-dev flex bison gettext autoconf libxml-parser-perl xsltproc libxrandr-dev python3-libxml2 nasm pkg-config git intltool checkinstall 
...
if [[ "$addencoding" = *"yes"* ]];
then
echo /bin/echo -e "\e[1;33m   |-| Installing encoding H264 & x264 packages       \e[0m" 
sudo apt-get -y install libopenh264-dev
...
if [[ "$addencoding" = "yes" ]];
then
./configure --enable-fuse --enable-jpeg --enable-rfxcodec --enable-mp3lame --enable-vsock --enable-painter --enable-openh264 --enable-x264 
else 
./configure --enable-fuse --enable-jpeg --enable-rfxcodec --enable-mp3lame --enable-vsock --enable-painter
fi
make
...
#Additional Configuration for xrdp to run as non-root
echo
/bin/echo -e "\e[1;32m       |-|  Additional settings for non-root service access xrdp     \e[0m" 
echo
#create local user  to run xrdp as non root 
...
sudo sed -i "s/#runtime_user=xrdp/runtime_user=xrdp/g" /etc/xrdp/xrdp.ini
sudo sed -i "s/#runtime_group=xrdp/runtime_group=xrdp/g" /etc/xrdp/xrdp.ini

#Edit /etc/xrdp/sesman.ini and uncomment this line:-
sudo sed -i "s/#SessionSockdirGroup=xrdp/SessionSockdirGroup=xrdp/g" /etc/xrdp/sesman.ini
...
#Check if change completed correctly 
...
sudo sed -i 's/.*ls_bg_color=.*/ls_bg_color=ffffff/' /etc/xrdp/xrdp.ini
...
do
    ...
    then
                ...
                echo " --encoding or -e         add H264 encoding capability when compiling"
                echo 
                echo "example                                                      "
                echo     
